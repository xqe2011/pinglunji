...
lastDanmuMessages = []
def filterDanmu(uid, uname, isFansMedalBelongToLive, fansMedalLevel, isFansMedalVIP, msg, isEmoji):
    ...
    if dynamicConfig["filter"]["danmu"]["fansMedalLevelBigger"] != 0 and fansMedalLevel < dynamicConfig["filter"]["danmu"]["fansMedalLevelBigger"]:
        return False
    if dynamicConfig["filter"]["danmu"]["isFansMedalVIP"] and not isFansMedalVIP:
        return False
...
def filterWelcome(uid, uname, isFansMedalBelongToLive, fansMedalLevel, isFansMedalVIP):
    dynamicConfig = getJsonConfig()['dynamic']
    ...
    if dynamicConfig["filter"]["welcome"]["fansMedalLevelBigger"] != 0 and fansMedalLevel < dynamicConfig["filter"]["welcome"]["fansMedalLevelBigger"]:
        return False
    if dynamicConfig["filter"]["welcome"]["isFansMedalVIP"] and not isFansMedalVIP:
        return False
    return True
