...
async def handleTTSSpeakerPlus():
    timeLog('[Keyboard] Trigging TTS speaker plus')
    nowJsonConfig = getJsonConfig()
...
async def handleReadLastGiftMessages():
    ...
    await readHistoryByType(['gift'], True)

async def initalizeKeyboard():
    runningLoop = asyncio.get_running_loop()
    keyboard.add_hotkey('shift+alt+f5', lambda: asyncio.run_coroutine_threadsafe(handleFlush(), runningLoop))
    keyboard.add_hotkey('shift+alt+f6', lambda: asyncio.run_coroutine_threadsafe(handleGetDelay(), runningLoop))

    keyboard.add_hotkey('shift+alt+[', lambda: asyncio.run_coroutine_threadsafe(handleTTSRatePlus(), runningLoop))
    keyboard.add_hotkey('shift+alt+]', lambda: asyncio.run_coroutine_threadsafe(handleTTSRateMinus(), runningLoop))

    keyboard.add_hotkey('shift+alt+=', lambda: asyncio.run_coroutine_threadsafe(handleTTSVolumePlus(), runningLoop))
    keyboard.add_hotkey('shift+alt+-', lambda: asyncio.run_coroutine_threadsafe(handleTTSVolumeMinus(), runningLoop))

    keyboard.add_hotkey('shift+alt+m', lambda: asyncio.run_coroutine_threadsafe(handleTTSSpeakerPlus(), runningLoop))

    keyboard.add_hotkey('shift+alt+f7', lambda: asyncio.run_coroutine_threadsafe(handleReadNextGiftMessages(), runningLoop))
    keyboard.add_hotkey('shift+alt+f8', lambda: asyncio.run_coroutine_threadsafe(handleReadNextHistoryDanmu(), runningLoop))
    keyboard.add_hotkey('shift+alt+t', lambda: asyncio.run_coroutine_threadsafe(handleReadLastGiftMessages(), runningLoop))
    keyboard.add_hotkey('shift+alt+y', lambda: asyncio.run_coroutine_threadsafe(handleReadLastHistoryDanmu(), runningLoop))
    keyboard.add_hotkey('shift+alt+f9', lambda: asyncio.run_coroutine_threadsafe(handleReadNewestMessages(), runningLoop))
